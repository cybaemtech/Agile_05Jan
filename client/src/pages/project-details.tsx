                                    ...
                                    <div className="flex flex-col">
                                      ...
                                      {item.type === 'EPIC' || item.type === 'FEATURE' || item.type === 'STORY' ? (
                                        <div className="flex gap-2 text-[10px] text-neutral-500 mt-0.5">
                                          {(() => {
                                            const totalEstimate = calculateTotalEstimatedHours(item.id, item.type);
                                            return totalEstimate > 0 ? (
                                              <span>Est: <span className="font-medium text-blue-600">{totalEstimate.toFixed(1)}h</span></span>
                                            ) : null;
                                          })()}
                                          {(() => {
                                            const totalActual = calculateTotalActualHours(item.id, item.type);
                                            return totalActual > 0 ? (
                                              <span>Act: <span className="font-medium text-orange-600">{totalActual.toFixed(1)}h</span></span>
                                            ) : null;
                                          })()}
                                        </div>
                                      ) : (
                            ...
                            <td className="px-2 py-1">
                              {editingCell?.itemId === item.id && editingCell?.field === 'actualHours' ? (
                                ...
                                if (item.type === 'EPIC' || item.type === 'FEATURE' || item.type === 'STORY') {
                                  ...
                                  // Show individual actual hours for task/bug items (include 0 values)
                                  return (item.actualHours !== null && item.actualHours !== undefined && item.actualHours !== '') ? (
                                    <span
