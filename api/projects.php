...
function getWorkItems($conn, $projectId) {
    try {
        ...
        $items = array_map(function($i) {
            ...
            $screenshot = null;
            if (isset($i['screenshot_blob']) && $i['screenshot_blob']) {
                $screenshot = 'data:image/png;base64,' . base64_encode($i['screenshot_blob']);
            } elseif (isset($i['screenshot_path']) && $i['screenshot_path']) {
                $screenshot = $i['screenshot_path'];
            } elseif (isset($i['screenshot']) && $i['screenshot']) {
            ...
            return [
                ...
                'estimate' => $i['estimate'] ? (int)$i['estimate'] : null,
                'actualHours' => isset($i['actual_hours']) ? (float)$i['actual_hours'] : null,
                'startDate' => $i['start_date'],
                'endDate' => $i['end_date'],
                'githubUrl' => $i['github_url'] ?? null,
                'bugType' => $i['bug_type'] ?? null,
                'severity' => $i['severity'] ?? null,
                'referenceUrl' => $i['reference_url'] ?? null,
                'currentBehavior' => $i['current_behavior'] ?? null,
                'expectedBehavior' => $i['expected_behavior'] ?? null,
